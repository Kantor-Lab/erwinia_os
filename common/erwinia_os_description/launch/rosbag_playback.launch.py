...
def launch_setup(context, *args, **kwargs):
    ...
    rviz_node = Node(
        ...
        executable='rviz2',
        name='rviz2',
        arguments=['-d', os.path.join(description_pkg, 'rviz', 'rosbag_view.rviz')],
        output='screen',
        parameters=[{'use_sim_time': True}]
    )

    # Build the frame prefix from platform_ns and platform_prefix
    # e.g. with ns='amiga' and prefix='' -> 'amiga/'
    _ns  = platform_ns.perform(context)
    _pfx = platform_prefix.perform(context)
    _fp  = f'{_ns}/{_pfx}' if _ns else _pfx  # e.g. 'amiga/'

    # Derives wheel joint positions from /odom twist and publishes JointState
    # so robot_state_publisher can animate the wheel rotation.
    odom_to_wheel_joints_node = Node(
        package='erwinia_os_description',
        executable='odom_to_wheel_joints.py',
        name='odom_to_wheel_joints',
        output='screen',
        parameters=[{
            'use_sim_time': True,
            'wheel_radius': 0.2159,
