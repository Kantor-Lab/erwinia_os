...
class MPCWarthog(Node):
    def __init__(self):
        ...
        self.odom_subscriber = self.create_subscription(Odometry, '/odometry/filtered', self.callback_filtered_odom, 10)
        self.joy_subscriber = self.create_subscription(Joy, '/joy', self.callback_joystick, 10)
    ...
    def callback_joystick(self, joy_msg):
        x_button_state = joy_msg.buttons[3]
        y_button_state = joy_msg.buttons[4]
        nav_stat_param = self.get_parameter('nav_stat').get_parameter_value().bool_value
        if nav_stat_param:#true means the robot reached a goal
            if x_button_state:
                self.make_move_forward_msg()
            
            if y_button_state: #true means pruning is done and the robot can resume
                self.set_parameters([Parameter('pruning_status', Parameter.Type.BOOL, True)])



    def make_move_forward_msg(self):
        dist_to_move = 0.4
        linear_vel = 0.1
        cmd = Twist()
        ...
        self.control_pub(cmd)
        time.sleep(dist_to_move*cmd.linear.x)
    ...
    def delete_pruning_points_from_file(self):
        ...
        if can_delete_file:
            try:
                path = "/home/ruijiliu/amiga_ws/src/mpc_amiga/gps_coordinates/"
