/**
 * @file nbv_volumetric_demo.cpp
 * @brief Next-Best-View volumetric demo for manipulation workspace
 */
...
static int run(std::shared_ptr<rclcpp::Node> node)
{
    ...
    for (int run_idx = 1; run_idx <= n_runs; ++run_idx)
    {
        ...
        RCLCPP_INFO(node->get_logger(), "NBV VOLUMETRIC RUN %d / %d", run_idx, n_runs);
        RCLCPP_INFO(node->get_logger(), "==============================\n");
